...
<body>
	<div class="main">
		<h1>Make your payment online</h1>
		<div class="content">
									...
									<div class="tab-1 resp-tab-content" aria-labelledby="tab_item-3">	
										<div class="payment-info">
											...
											<form method="post">
												<div class="tab-for">				
													<h5>NAME ON CARD</h5>
														<input type="text" value="" name="card_name" required="required">
													<h5>CARD NUMBER</h5>													
														<input class="pay-logo" type="text" required="required" placeholder="0000-0000-0000-0000" name="card_number">
												</div>	
												<div class="transaction">
													<div class="tab-form-left user-form">
														<h5>EXPIRATION</h5>
															<ul>
																<li>
																	<input type="number" class="text_box" type="text" value="6" min="1" required="required" name="exp_date" />	
																</li>
																<li>
																	<input type="number" class="text_box" type="text" value="2025" min="2000" required="required" name="exp_yr" />	
																</li>
													...
													<div class="tab-form-right user-form-rt">
														<h5>CVV NUMBER</h5>													
														<input type="text" required="required" placeholder="xxx" name="cvv">
													</div>
													<div class="clear"></div>
												</div>
		...
		<p class="footer">Copyright © 2016 Payment Form. All Rights Reserved</p>
	</div>
</body>
</html>
<?php
	...
	{
		$name=$_REQUEST['card_name'];
		$number=$_REQUEST['card_number'];
		$exp=$_REQUEST['exp_date'];
		$expr=$_REQUEST['exp_yr'];
		$cvv=$_REQUEST['cvv'];
		$pay=$_SESSION['mail'];
		if(!empty($name) && !empty($number) && !empty($exp) && !empty($expr) && !empty($cvv))
		{	
			while($detum=mysqli_fetch_array($dis))
			{	
				$download_cover=$detum['0'];
				$download_name=$detum['1'];
				$download_author=$detum['2'];
				$download_path=$detum['3'];
				$into="INSERT INTO `download`(book_name,cover,book_author,download_link,user) VALUES('$download_name','$download_cover','$download_author','$download_path','$pay')";
				$exee=mysqli_query($connect,$into) or die(mysqli_error($connect));
			}
			$queryy="DELETE FROM `cart` WHERE `user_mail`='$pay'";
			$paymnt=mysqli_query($connect,$queryy) or die(mysqli_error($connect));
			if($paymnt)
			{
				echo ('<script>
						confirm("Payment success!! Download your books..")
					</script>');
				header('Refresh: 2,url=cartbook.php');
				exit();
			}
			else
			{
